	...
	xhr.onreadystatechange = function(){
		if(xhr.readyState == 4 && xhr.status == 200){
			...
			document.getElementById("resolveApprove").onclick = function(){
				//console.log("SHOIULD BE CORRECT PARENT NODE: " + a.parentNode.getElementsByTagName("td")[0].innerHTML);
				let resolveId = a.parentNode.getElementsByTagName("td")[0].innerHTML;
				for(var r of reimbs){
					//console.log("FOR NEW APPROVAL CURRENT REIMB_ID: " + r.reimb_id + " CURRENT RESOLVEID: " + resolveId);
					if(r.reimb_id == resolveId){
						...
						//loadFinanceManagerView(correctUser, users);
						//loadFinanceManagerView(correctUser, users);
			...
			document.getElementById("resolveDeny").onclick = function(){
				let resolveId = a.parentNode.getElementsByTagName("td")[0].innerHTML;
				for(var r of reimbs){
					//console.log("FOR NEW DENIAL CURRENT REIMB_ID: " + r.reimb_id + " CURRENT RESOLVEID: " + resolveId);
					if(r.reimb_id == resolveId){
						var request=new XMLHttpRequest();
						request.open("POST", "financeManager" , true);
						request.setRequestHeader("Content-type", "application/x-www-form-urlencoded"); 
				...
				request.open("POST", "financeManager" , true);
				request.setRequestHeader("Content-type", "application/x-www-form-urlencoded"); 
				request.send(r.reimb_status_id);
				*/
			}
